...
( cd "${MAROONEDPODS_DIR}/tools/manifest-generator/" && GO111MODULE=${GO111MODULE:-off} go build -o "${generator}" -buildvcs=false ./... )

echo "DOCKER_PREFIX=${DOCKER_PREFIX}"
echo "DOCKER_TAG=${DOCKER_TAG}"
echo "VERBOSITY=${VERBOSITY}"
echo "PULL_POLICY=${PULL_POLICY}"
echo "MAROONEDPODS_NAMESPACE=${MAROONEDPODS_NAMESPACE}"
source "${script_dir}"/resource-generator.sh

mkdir -p "${MANIFEST_GENERATED_DIR}/"

#generate operator related manifests used to deploy maroonedpods with operator-framework
