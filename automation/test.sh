...
export MAROONEDPODS_NAMESPACE="maroonedpods-$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 5 | head -n 1)"

echo "maroonedpods-namespace: ${MAROONEDPODS_NAMESPACE}"

if [[ -n "$RANDOM_CR" ]]; then
  export CR_NAME="${MAROONEDPODS_NAMESPACE}"
fi

readonly ARTIFACTS_PATH="${ARTIFACTS}"
readonly BAZEL_CACHE="${BAZEL_CACHE:-http://bazel-cache.kubevirt-prow.svc.cluster.local:8080/maroonedpods.io}"
...
export MAROONEDPODS_NAMESPACE="${MAROONEDPODS_NAMESPACE:-maroonedpods}"
