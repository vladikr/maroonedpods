...
func generateGoFile(outputDir string, crds map[string]*extv1.CustomResourceDefinition) {
	...
	writeOrPanic(file, "// MaroonedPodsCRDs is a map containing yaml strings of all CRDs\n")
	writeOrPanic(file, "var MaroonedPodsCRDs map[string]string = map[string]string{\n")
