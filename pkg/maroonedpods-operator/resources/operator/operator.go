...
const (
	roleName        = "maroonedpods-operator"
...
func createMaroonedPodsListCRD() *extv1.CustomResourceDefinition {
	...
	_ = k8syaml.NewYAMLToJSONDecoder(strings.NewReader(resources.MaroonedPodsCRDs["maroonedpods"])).Decode(&crd)
	return &crd
}
...
func createOperatorEnvVar(operatorVersion, deployClusterResources, controllerImage, webhookServerImage, verbosity, pullPolicy string) []corev1.EnvVar {
	return []corev1.EnvVar{
		...
		{
			Name:  "MAROONEDPODS_SERVER_IMAGE",
...
func createClusterServiceVersion(data *ClusterServiceVersionData) (*csvv1.ClusterServiceVersion, error) {
	...
	description := `
MaroonedPods schedules pods on a dedicated VirtualMachines acting as nodes and provided by KubeVirt

_The MAroonedPods Operator does not support updates yet._
`
	...
	deployment.Spec.Template.Spec.PriorityClassName = utils2.MaroonedPodsPriorityClass
	...
	return &csvv1.ClusterServiceVersion{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      "maroonedpodsoperator." + data.CsvVersion,
      ...
      [
        {
          ...
          "spec": {
            "imagePullPolicy":"IfNotPresent"
				...
				"description": "Creates and maintains MaroonedPods deployments",
			},
		},

		Spec: csvv1.ClusterServiceVersionSpec{
			DisplayName: "MaroonedPods",
			Description: description,
			Keywords:    []string{"MaroonedPods", "Virtualization", "MultiTenancy"},
			...
			Provider: csvv1.AppLink{
				Name: "MaroonedPods project",
			},
			Links: []csvv1.AppLink{
				{
					Name: "MaroonedPods",
					URL:  "https://github.com/vladikr/maroonedPods",
				},
				{
					Name: "Source Code",
					URL:  "https://github.com/vladikr/maroonedPods",
			...
			CustomResourceDefinitions: csvv1.CustomResourceDefinitions{
				...
				Owned: []csvv1.CRDDescription{
					{
						Name:        "maroonedpods.io",
						Version:     "v1alpha1",
						Kind:        "MaroonedPods",
						DisplayName: "MaroonedPods deployment",
						Description: "Represents a maroonedpods deployment",
						Resources: []csvv1.APIResourceReference{
							{
								Kind:    "ConfigMap",
								Name:    "maroonedpods-operator-leader-election-helper",
								Version: "v1",
							},
						},
						SpecDescriptors: []csvv1.SpecDescriptor{
							...
							{
								Description:  "The ImageRegistry to use for the MaroonedPods components.",
								DisplayName:  "ImageRegistry",
								Path:         "imageRegistry",
								XDescriptors: []string{"urn:alm:descriptor:text"},
							},
							{
								Description:  "The ImageTag to use for the MaroonedPods components.",
								DisplayName:  "ImageTag",
								Path:         "imageTag",
								XDescriptors: []string{"urn:alm:descriptor:text"},
							},
							{
								Description:  "The ImagePullPolicy to use for the MaroonedPods components.",
						...
						StatusDescriptors: []csvv1.StatusDescriptor{
							...
							{
								Description:  "Explanation for the current status of the MAroonedPods deployment.",
								DisplayName:  "Conditions",
								Path:         "conditions",
								XDescriptors: []string{"urn:alm:descriptor:io.kubernetes.conditions"},
							},
							{
								Description:  "The observed version of the MaroonedPods deployment.",
								DisplayName:  "Observed MAroonedPods Version",
								Path:         "observedVersion",
								XDescriptors: []string{"urn:alm:descriptor:text"},
							},
							{
								Description:  "The targeted version of the MaroonedPods deployment.",
								DisplayName:  "Target MaroonedPods Version",
								Path:         "targetVersion",
								XDescriptors: []string{"urn:alm:descriptor:text"},
							},
							{
								Description:  "The version of the MaroonedPods Operator",
								DisplayName:  "MaroonedPods Operator Version",
